...
with col1:
    ...
    folium.GeoJson(muni_sel, style_function=lambda x: {"color":"#0000ff","weight":2,"fill":False}).add_to(m)
    folium.GeoJson(desm_sel, style_function=lambda x: {"color":"red","weight":1,"fillOpacity":0.6}).add_to(m)
...
if st.checkbox("Mostrar mapa nacional"):
    m2 = folium.Map(location=[-10,-55], zoom_start=4, control_scale=True)
    folium.GeoJson(prodes, style_function=lambda x: {"color":"darkgreen","weight":0.5,"fillOpacity":0.4}).add_to(m2)
